...
if (!empty($_POST)) {
    ...
    // メール送信
    if ($isComplete && !isset($_SESSION['sendMail'])) {
        if (!empty($_POST["send"])) {
            if (getenv("SEND_METHOD") == "sendmail") {
                $formTool->sendReturnMailSendMail();
                $formTool->sendReturnAdminMailSendMail();
            }

            if (getenv("SEND_METHOD") == "smtp") {
                $formTool->sendReturnMail();
                $formTool->sendReturnAdminMail();
            }
        }
    }
}
...
if (isset($_SESSION['sendMail'])) {
    ...
    $_POST       = [];
    $redirectUrl = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'] . '/contact/';
...
<article id="warp">
    ...
    <section class="thanks">
        <div>送信完了</div>
        <p>
            この度はご相談いただき<br>ありがとうございます。
            <br><br>
            確認のため、お客さまに<br>自動送信メールを<br>お送りしております。
            <br><br>
            お申込みいただいた内容にて、<br>後日、担当者より<br>ご案内いたします。
        </p>
        <a class="top_back" href="/">TOPへ戻る</a>
    </section>
</article>

<?php $_SESSION['sendMail'] = 1; ?>
<input type="hidden" id="pageKind" name="pageKind" value="3">
